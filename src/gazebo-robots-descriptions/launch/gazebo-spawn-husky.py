...
def generate_launch_description(): 
    ...
    cmd_vel_bridge = SingleSubstitution("/model/{@}/cmd_vel@geometry_msgs/msg/Twist@gz.msgs.Twist", 
                                        "{@}", 
                                        ns)
    point_cloud_bridge = SingleSubstitution("/world/empty/model/{@}"\
                                            "/link/sensor_rack/sensor/front_lidar/scan/points@"\
                                            "sensor_msgs/msg/PointCloud2[gz.msgs.PointCloudPacked", 
                                            "{@}", 
                                            ns)
    lidar_scan_bridge = SingleSubstitution("/world/empty/model/{@}"\
                                           "/link/sensor_rack/sensor/front_lidar/scan/points@"\
                                           "sensor_msgs/msg/LaserScan[gz.msgs.LaserScan",
                                           "{@}", 
                                            ns)
    imu_bridge = SingleSubstitution("/world/empty/model/{@}"\
                                    "link/sensor_rack/sensor/imu_sensor/imu@"\
                                    "sensor_msgs/msg/Imu[gz.msgs.IMU",
                                    "{@}", 
                                    ns)
    odometry_bridge = SingleSubstitution("/model/{@}"\
    ...
    ros_bridge_node = create_node_description(
                        ...
                        remappings=[
                            (pyexp(SingleSubstitution('/model/{@}/cmd_vel','{@}', ns)), 
                             pyexp(SingleSubstitution('/{@}/cmd_vel', '{@}', ns))),
                            (pyexp(SingleSubstitution('/world/empty/model/{@}/link/sensor_rack/sensor/front_lidar/scan/points', ns)), 
                             pyexp(SingleSubstitution('/{@}/lidar/points', "{@}", ns))),
                            (pyexp(SingleSubstitution('/world/empty/model/{@}/link/sensor_rack/sensor/front_lidar/scan', "{@}", ns)), 
                             pyexp(SingleSubstitution('/{@}/lidar/scan', "{@}", ns))),
                            (pyexp(SingleSubstitution('/world/empty/model/{@}/link/sensor_rack/sensor/imu_sensor/imu', "{@}", ns)), 
                             pyexp(SingleSubstitution('/{@}/imu', "{@}", ns))),
                            (pyexp(SingleSubstitution('/model/{@}/odometry', "{@}", ns)), 
                             pyexp(SingleSubstitution('/{@}/odometry', "@", ns)))
                        ],
    )
